...
* 1 уровень: [L4](https://dzen.ru/a/Zsm12k4R6gXwmfpV#:~:text=%D0%91%D0%B0%D0%BB%D0%B0%D0%BD%D1%81%D0%B8%D1%80%D0%BE%D0%B2%D1%89%D0%B8%D0%BA%20%D0%BD%D0%B0%20L4%20%D0%BF%D1%80%D0%B8%D0%BD%D0%B8%D0%BC%D0%B0%D0%B5%D1%82%20%D1%80%D0%B5%D1%88%D0%B5%D0%BD%D0%B8%D1%8F%20%D0%BD%D0%B0%20%D0%BE%D1%81%D0%BD%D0%BE%D0%B2%D0%B5%20%D0%B0%D0%BB%D0%B3%D0%BE%D1%80%D0%B8%D1%82%D0%BC%D0%B0%20%D0%B1%D0%B0%D0%BB%D0%B0%D0%BD%D1%81%D0%B8%D1%80%D0%BE%D0%B2%D0%BA%D0%B8%20%D0%B8%20%D0%B8%D0%BD%D1%84%D0%BE%D1%80%D0%BC%D0%B0%D1%86%D0%B8%D0%B8%20%D0%BE%20TCP/UDP%2D%D1%81%D0%B5%D1%81%D1%81%D0%B8%D1%8F%D1%85%2C%20%D0%B2%D0%BA%D0%BB%D1%8E%D1%87%D0%B0%D1%8F%20IP%2D%D0%B0%D0%B4%D1%80%D0%B5%D1%81%D0%B0%2C%20%D0%BF%D0%BE%D1%80%D1%82%D1%8B%20%D0%B8%D1%81%D1%82%D0%BE%D1%87%D0%BD%D0%B8%D0%BA%D0%B0%20%D0%B8%20%D0%BD%D0%B0%D0%B7%D0%BD%D0%B0%D1%87%D0%B5%D0%BD%D0%B8%D1%8F.), Балансировщик на L4 принимает решения на основе алгоритма балансировки и информации о TCP/UDP-сессиях, включая IP-адреса, порты источника и назначения.
* 2 уровень: [L7](https://habr.com/ru/companies/vk/articles/347026/), Application Load Balancers (например, NGINX Ingress Controller) ([Источник](https://networkguru.ru/balansirovshhiki-nagruzki-application-load-balancer-i-network-load-balancer-v-amazon-web-services-aws/)) маршрутизируют запросы к отдельным приложениям или микросервисам.
...
| **posts**               | `group_id mod N`               | Распределение постов для ускорения записи и чтения               | Репликационный фактор = 3 в Cassandra                |
...
#### Чуть подробнее про ML-ранжирование

Персональное ML‑ранжирование — это конвейер, который из огромного множества постов сначала отбирает кандидатов, а потом с помощью модели (например, градиентного бустинга CatBoost) упорядочивает их под конкретного пользователя.

##### 1. Сбор сигналов

Система постоянно логирует взаимодействия пользователя с контентом: просмотры, клики, лайки, репосты, скрытия, время чтения (dwell time), отписки. Эти события собираются в логи/ClickHouse и используются как обучающая выборка: для каждой пары «пользователь–пост» известно, был ли клик/лайк и насколько долго пользователь смотрел пост.

##### 2. Формирование признаков

Для каждой пары «пользователь–пост» считаются фичи:  
- признаки пользователя (темы интересов, активность, гео, платформа);  
- признаки поста (тип контента, длина, время создания, автор, тематика);  
- признаки взаимодействия пользователя и автора/темы в прошлом (сколько постов этого автора пользователь открывал, CTR по этой теме и т.д.).
Также формируются «таргеты» — целевые метки, например «кликнул», «досмотрел/дочитал до конца», «поставил лайк».

##### 3. Обучение модели ранжирования

На этих данных обучается модель (градиентный бустинг или нейросеть), которая для набора кандидатов учится ставить более релевантные посты выше. Оптимизируются метрики ранжирования вроде NDCG/MAP: модель наказывается, если интересный для пользователя пост оказался низко в выдаче. Модель периодически переобучают на свежих логах, чтобы подстраиваться под изменяющееся поведение.

##### 4. Онлайн‑работа:

В онлайне у пользователя есть запрос «дай ленту»:  
1. Быстрый этап candidate generation выбирает несколько сотен–тысяч постов (например, новые посты друзей, популярные посты по интересам). Это делают простые эвристики и/или отдельные модели‑recall, чтобы не гонять тяжёлую модель по всей базе.
2. Для этих кандидатов вычисляются признаки «на лету», и ML‑модель считает скор релевантности для текущего пользователя.
3. Посты сортируются по скору, сверху могут применяться правила про разнообразие/свежесть/ограничения (не больше N постов от одного автора), и итоговый список отправляется в фид.

##### 5. Отличие от простых эвристик

По сути, система учится на реальном поведении людей: какие посты они открывают, дочитывают, лайкают, а какие пролистывают, и на этой базе максимально персонализирует порядок. В отличие от фиксированных правил (по времени/лайкам), ML‑ранкер учитывает десятки–сотни факторов одновременно и сам находит нелинейные зависимости, за счёт чего даёт заметно более высокий CTR и вовлечённость ленты.
...
Точка входа в сам сервис - API Gateway. Он же в свою очередь ходит по микросервисам и собирает необходимую информацию. 

Несколько комментариев для пояснения работы некоторых блоков:
1. Микросервис авторизации - ходит в микросервис Users для получения инфы о пользователях, затем идет в редис с целью хранения сессии пользователя.
2. Микросервисы Групп, Друзей и Мессенджера через Кафку дружат с микросервисом Уведомлений. Это обеспечивает надежный механизм оповещения пользователей о новых сообщениях, заявках в друзья и приглашениях в группы в состоянии офф-лайн.
